...
cron_services=("cron-update.sh" "postgres-listen" "stop-abandoned-user-sessions")
SshUser=$1
text_payload=""
...
for t in ${systemctl_service[@]}; do
  echo $(sudo systemctl status ${t} |grep "active (running)")
  check=$(sudo systemctl status ${t} |grep -c "active (running)")
        if [ "$check" -lt "1" ]; then
                text_payload=""$text_payload" \n "${t}" check failed"
                let errors+=1
        else
...
for v in ${cron_services[@]}; do
  check=$(sudo -u "$SshUser" crontab -l |grep -c ${v} ;exit 0)
        ...
        if [ "$check" -eq "1" ]; then
                ...
                secondCheck=$(sudo -u "$SshUser"  crontab -l |grep ${v} |grep -c "#" ;exit 0)
...
export_json(){
        ...
        gcloud logging write some-log "$json_payload" --payload-type=json
}
