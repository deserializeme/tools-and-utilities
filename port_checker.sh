...
do
	...
	if [ ! -z "$ss" ]; then
		...
		result=$(sudo -u strivr-admin ssh -o StrictHostKeyChecking=no -p $port strivr-admin@localhost)
		...
		if [ ! -z "$ss" ]; then
			...
			if [ ! -z "$PIDS" ]; then
				...
				result2=$(sudo -u strivr-admin ssh -o StrictHostKeyChecking=no -p $port strivr-admin@localhost)
				netstat -ant |grep "$port"
				sudo lsof -ti:$port
			else
